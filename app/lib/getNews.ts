...
/*
 * Ovo je funkcija koja STVARNO ide na Google Sheets.
 *
 * Ne pozivamo je direktno sa stranica.
 */
async function getNewsFromSheets(): Promise<NewsItem[]> {
	const spreadsheetId = process.env.GOOGLE_SHEETS_ID;
 ...
 * Next cache.
 *
 * Google Sheets će biti pozvan najviše
 * jednom u 2 minuta za ovaj podatak.
 */
const getCachedNews = unstable_cache(getNewsFromSheets, ["itnms-news"], {
	revalidate: 120,
	tags: ["news"],
});

/*
 * Ovu funkciju ostatak sajta koristi kao i do sada.
 *
 * Dakle NE moraš da menjaš:
 *
